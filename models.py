...
def init_db():
    ...
    db.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS productos (
            ...
            pincentral_fields TEXT DEFAULT '',
            pincentral_recarga_cantidad INTEGER DEFAULT 1,
        ...
        CREATE TABLE IF NOT EXISTS usuario_api_productos_bloqueados (
            usuario_id INTEGER NOT NULL,
            producto_id INTEGER NOT NULL,
            fecha TEXT DEFAULT (datetime('now','localtime')),
    ...
    try:
        db.execute("SELECT usa_pincentral FROM productos LIMIT 1")
    except Exception:
        db.execute("ALTER TABLE productos ADD COLUMN usa_pincentral INTEGER DEFAULT 0")
    try:
    ...
    try:
        db.execute("SELECT pincentral_fields FROM productos LIMIT 1")
    except Exception:
        db.execute("ALTER TABLE productos ADD COLUMN pincentral_fields TEXT DEFAULT ''")
    try:
        db.execute("SELECT pincentral_recarga_cantidad FROM productos LIMIT 1")
    except Exception:
        db.execute("ALTER TABLE productos ADD COLUMN pincentral_recarga_cantidad INTEGER DEFAULT 1")
    try:
    ...
    db.execute("INSERT OR IGNORE INTO configuracion (clave, valor) VALUES ('telegram_precios_bot_token', '')")
    db.execute("INSERT OR IGNORE INTO configuracion (clave, valor) VALUES ('telegram_precios_chat_id', '')")
    db.execute("INSERT OR IGNORE INTO configuracion (clave, valor) VALUES ('telegram_precios_activo', '0')")
